...
func deleteMeHandler(c *gin.Context) {
	...
	if _, err := tx.Exec(ctx, queries.UsersNullifyProjectsCreatedBy, userID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	if _, err := tx.Exec(ctx, queries.UsersNullifyTenantsCreatedBy, userID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	// memberships 会通过 ON DELETE CASCADE 自动清理
	if _, err := tx.Exec(ctx, queries.UsersDeleteEmailVerifications, userID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	if _, err := tx.Exec(ctx, queries.UsersDeleteUser, userID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
